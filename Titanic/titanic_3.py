...
g = sns.distplot(dataset['Fare'], color='b', label=f"Skewness: {dataset['Fare'].skew():.2f}")
g = g.legend(loc='best')
plt.show()
...
# Embarked
dataset['Embarked'] = dataset['Embarked'].fillna('S', inplace=True)
...
g = sns.factorplot(y='Survived', x='Cabin', data=dataset, kind='bar', order=['A', 'B', 'C', 'D', 'E',
                                                                             'F', 'G', 'T', 'X'])
g = g.set_ylabels('Survival Probability')
plt.show()
...
g = plot_learning_curve(gsSVMC.best_estimator_, 'ExtraTrees learning curves', X_train, Y_train, cv=kfold)
g = plot_learning_curve(gsadaDTC.best_estimator_, 'ExtraTrees learning curves', X_train, Y_train, cv=kfold)
g = plot_learning_curve(gsGBC.best_estimator_, 'ExtraTrees learning curves', X_train, Y_train, cv=kfold)
...
ensemble_result = pd.concat([test_Survived_RFC, test_Survived_ExtC, test_Survived_AdaC,
